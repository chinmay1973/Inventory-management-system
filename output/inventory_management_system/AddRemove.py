...
#========================================================================================================================
        self.but_1 = Button(self.main, text="Add", command=self.Add, font=('goudy old style', 12),
                            cursor='hand2', borderwidth=3, relief="raised")
        self.but_1.place(x=300, y=670)

        self.but_2 = Button(self.main, text="Add", command=self.Add2, font=('goudy old style', 12),
                            cursor='hand2', borderwidth=3, relief="raised")
        self.but_2.place(x=700, y=670)
        ...
        self.but_9 = Button(self.main, text="Remove", command=self.Remove1, font=('goudy old style', 12), cursor='hand2',
                            borderwidth=3, relief="raised", width=7, height=1)
        self.but_9.place(x=355, y=670)

        self.but_10 = Button(self.main, text="Remove", command=self.Remove2, font=('goudy old style', 12), cursor='hand2',
                            borderwidth=3, relief="raised", width=7, height=1)
        self.but_10.place(x=765, y=670)
...
#=========================================================================================================================
    def Add(self):
        try:
                    if self.GaneshProductEntryQty.get() <= 0:
                        ...
                        mycursor.execute(
                            f"insert into productdatatable (product,price,qty) values ('{self.GaneshProductEntry.get()}',{self.GaneshProductEntryPrice.get()},{self.GaneshProductEntryQty.get()}) ")
                    ...
                    for i in range(0, len(myresult), 1):
                        self.ProductTable.insert(parent='', index='end', iid=i,
                                                 values=(myresult[i][0], myresult[i][1], myresult[i][2]))
                    mydb.commit()
                    '''messagebox.showinfo('', 'Product is added)'''
        except:
            messagebox.showerror("Quantity ERROR!", " Enter a Valid Quantity!!")
    ...
    def Add2(self):
        try:
                if self.BirthdayproductEntryQty.get() <= 0:
                    ...
                    mycursor.execute(
                            f"insert into birthdaytable (product,price,qty) values ('{self.BirthdayproductEntry.get()}',{self.BirthdayProductEntryPrice.get()},{self.BirthdayproductEntryQty.get()})")
    ...
    def Remove1(self):
        mycursor.execute(f"delete from productdatatable where product = '{self.GaneshProductEntry.get()}'")
        qry1 = "select Product, Price, Qty from ProductDataTable"
        mycursor.execute(qry1)
        myresult = mycursor.fetchall()
        self.ProductTable.delete(*self.ProductTable.get_children())
    ...
    def Remove2(self):
        mycursor.execute(f"delete from birthdaytable where product = '{self.BirthdayproductEntry.get()}'")
        qry1 = "select Product, Price, Qty from birthdaytable"
        mycursor.execute(qry1)
        myresult = mycursor.fetchall()
        self.ProductTable2.delete(*self.ProductTable2.get_children())
